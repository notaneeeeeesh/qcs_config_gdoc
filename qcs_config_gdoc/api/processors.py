...
@frappe.whitelist()
def getClassifier(str):
    classifier = frappe.get_doc("Gdoc Classifier", "classifier_1")
    mime_type = str["mime"].split(":")[1].split(";")[0]
    classifierRequest = callClassifier(location = classifier.region, processor_id = classifier.classifier_id, project_id = classifier.project_name, authentication = classifier.authentication_file_path, mime = mime_type, image = str["content"])
    processor = None
    reference_doctype = None
    print(classifierRequest)
    for i in classifier.labels:
        if i.label == classifierRequest:
            processor = i.reference_processor
            reference_doctype = i.reference_doctype
            break
    if(processor == None):
        frappe.throw("Valid processor not found")
    else:
        call = getProc(instanceName = processor, str = str)
    return {"data": call, "doctype":reference_doctype}       
    # return call   
...
@frappe.whitelist()
def callClassifier(project_id,location,processor_id,authentication,image,mime):
    # os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = f"./{frappe.local.site}{authentication}"
    # content = image
    # mime_type = mime
    # opts = ClientOptions(api_endpoint=f"{location}-documentai.googleapis.com")
